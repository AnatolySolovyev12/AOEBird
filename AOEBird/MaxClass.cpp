...
void MaxClass::getstateInstance()
{
	QString tempUrl = urlForCheck;

	QUrl url(tempUrl.replace("checkAccount", "getStateInstance"));
	...
	QObject::connect(reply, &QNetworkReply::finished, [reply]() {
		...
		if (reply->error() == QNetworkReply::NoError)
		{
			...
			if (response.contains("authorized"))
				qDebug() << "GreenApi instance: MAX is authorized";
		}
		else
		{
			qDebug() << "Error:: " << reply->error();
